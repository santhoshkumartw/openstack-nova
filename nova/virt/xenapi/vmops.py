...
class VMOps(object):
    ...
    def spawn(self, instance, network_info):
        ...
        self._spawn(instance, vm_ref)
        self.plug_vifs(instance, vm_ref, network_info)
    ...
    def _destroy(self, instance, vm_ref, shutdown=True,
        ...
        self.unplug_vifs(instance, vm_ref)
    ...
    def unplug_vifs(self, instance, vm_ref):
        logging.debug(_("unplugging vif(s) for vm: |%s|"), vm_ref)
        ...
        for device in [0, 1, 2]:
            network_id = {
              2: FLAGS.nvp_service_network_id,
              1: FLAGS.nvp_public_network_id,
              0: tenant_network_id}[device]
            LOG.debug("Using network_id: %s" % network_id)
            attachment = "%s-eth%d" % (vm_name, device)
    ...
    def plug_vifs(self, instance, vm_ref, network_info):
        logging.debug(_("plugging vif(s) for vm: |%s|"), vm_ref)
        ...
        for device, (network, info) in enumerate(network_info):
            LOG.debug("device: %s" % str(device))
            network_id = network['bridge']
            LOG.debug("Using network_id: %s" % network_id)
            port_id = quantum.create_port(tenant_id, network_id)
            quantum.plug_iface(tenant_id, network_id, port_id,
              "%s-eth%d" % (vm_name, device))

    def find_integration_bridge(self):
        # TODO(bgh): we need a more reliable way of figuring out the
        # intgration bridge here.  It could be different on different
        # hypervisors.
